...
import styles from "./AssignedOrders.module.scss";
...
const AssignedOrders = ({ user }) => {
  ...
  const getOrderDetail = async (row) => {
    ...
    const response = await getDetallesPedidoUsuario(row.id_pedido);
    const data = response.data;
    if (response.status === 200) {
  ...
  const handleUpdateOrderStatus = (row) => {
    setSelectedOrder(row.id_pedido);
